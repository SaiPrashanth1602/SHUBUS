...
import { Bus, MapPin, User, Phone, Hash, Users, CalendarClock, ArrowUp } from "lucide-react" 
...
function AddShuttle() {
  ...
  const filteredBuses = availableBuses.filter(bus => {
    ...
    const matchesPlate = bus.numberPlate ? bus.numberPlate.toLowerCase().includes(query) : false
    ...
    <PageWrapper role="admin">
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-6 gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium text-sm">{currentDateDisplay}</p>
        </div>
      </div>
      ...
      <section className="mb-8">
        ...
        {loading ? (
          ...
          <div className="bg-white border border-gray-100 p-12 rounded-2xl text-center shadow-sm flex flex-col items-center justify-center">
            <Bus size={48} className="text-gray-200 mb-4" />
            <p className="text-gray-500 font-bold text-lg leading-tight">No such buses available</p>
            <p className="text-gray-400 text-sm mt-2 max-w-[250px] mx-auto">Try searching for a different Bus No, Plate, Type or check assigned shuttles.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {filteredBuses.map(bus => (
              <div key={bus.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex justify-between items-center hover:shadow-md transition-shadow relative overflow-hidden">
                ...
                <div className="mt-1">
                  ...
                  <p className="text-xs text-gray-500 uppercase tracking-wider">{bus.numberPlate}</p>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => setViewBus(bus)} className="px-3 py-2 text-sm font-semibold border border-gray-300 text-gray-600 rounded-lg hover:bg-gray-100 transition">View</button>
                  <button onClick={() => setSelectedBus(bus)} className="bg-vitblue text-white text-sm font-bold px-4 py-2 rounded-lg hover:bg-blue-800 transition shadow-sm">Assign</button>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-2xl w-full max-w-md animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <p className="text-sm text-gray-500">Assigning {selectedBus.busNo} for <strong>{isSchedulingForTomorrow ? "Tomorrow" : "Today"}</strong>.</p>
              </div>
              <button onClick={() => setSelectedBus(null)} className="p-2 hover:bg-slate-100 rounded-full text-slate-500 transition">✕</button>
            </div>
            <div className="space-y-4">
              <select className="w-full p-3 border rounded-xl bg-gray-50 outline-none font-semibold text-gray-700" value={route} onChange={e => setRoute(e.target.value)}>
                <option value="">Select Route</option>
                {ROUTES.map(r => <option key={r} value={r}>{r}</option>)}
              </select>
              <select className="w-full p-3 border rounded-xl bg-gray-50 outline-none font-semibold text-gray-700" value={time} onChange={e => setTime(e.target.value)}>
                <option value="">Select Time</option>
                {TIMES.map(t => <option key={t} value={t}>{t}</option>)}
              </select>
              <div className="flex gap-3 pt-4">
                <button onClick={() => setSelectedBus(null)} className="flex-1 py-3 text-gray-600 font-semibold hover:bg-gray-100 rounded-xl">Cancel</button>
                <button onClick={handleAddShuttle} className="flex-1 bg-vitblue text-white py-3 rounded-xl font-bold shadow-lg">Confirm</button>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-6">
            <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 space-y-3">
              ...
              <div className="flex justify-between items-center py-1">
                ...
                }`}>{viewBus.busType}</span>
              </div>
              <KV icon={<Users size={16} className="text-slate-400" />} label="Capacity" value={`${viewBus.totalSeats || 50} Seats`} />
            </div>
      ...
      <h2 className="text-lg font-semibold mb-4 mt-8 flex items-center gap-2">
        ...
        <span className="bg-green-100 text-green-700 text-[10px] uppercase font-bold tracking-wider px-2 py-1 rounded-full">Live</span>
      </h2>
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="overflow-x-auto scrollbar-hide">
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="bg-gray-50 text-gray-400 text-[10px] uppercase tracking-widest font-bold border-b border-gray-100">
                <th className="p-4 whitespace-nowrap">Bus</th>
                <th className="p-4 whitespace-nowrap">Morning Route</th>
                <th className="p-4 whitespace-nowrap">Plate</th>
                <th className="p-4 whitespace-nowrap">Route (Today)</th>
                <th className="p-4 whitespace-nowrap">Time</th>
                <th className="p-4 whitespace-nowrap">Driver</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {shuttles.map(s => {
                ...
                return (
                  <tr key={s.id} className="hover:bg-blue-50/50 transition-colors">
                    ...
                    <td className="p-4 text-gray-600 whitespace-nowrap">{bus?.morningRoute || "—"}</td>
                    <td className="p-4 text-gray-500 font-mono text-xs whitespace-nowrap uppercase">{bus?.numberPlate || "—"}</td>
                    <td className="p-4 text-gray-800 font-medium whitespace-nowrap">{s.route}</td>
                    <td className="p-4"><span className="bg-green-100 text-green-700 px-2 py-1 rounded text-xs font-bold">{s.time}</span></td>
                    <td className="p-4 text-gray-600 whitespace-nowrap">{bus?.driver?.name || "—"}</td>
              ...
              {shuttles.length === 0 && (
                <tr><td colSpan="7" className="p-8 text-center text-gray-400 font-medium">No shuttles active {isSchedulingForTomorrow ? "for tomorrow" : "yet"}.</td></tr>
              )}
            </tbody>
          </table>
        </div>
